...
# def create_dataset(df:np.ndarray, look_back:int, horizon:int=1, mode:str=CLF)->tuple[np.ndarray, np.ndarray]:
#     size = df.shape[0] - look_back - horizon + 1
#     X = np.array([df[i : i+look_back] for i in range(size)])
#     y = create_label(df, mode)
#     y = np.array([y[i+look_back : i+look_back+horizon] for i in range(size)])
#     return X, y

# def get_data(look_back:int, horizon:int=1, test_size:float=0.8, batch_size:int=32, data_dir:str=RAW_DATA_DIR, mode:str=CLF) -> dict[str, dict[str, list[np.ndarray]]]:
#     cid = 0
#     data_dict:dict[str, dict[str, pd.DataFrame]] = {}
#     for file_name in os.listdir(data_dir):
#         if file_name.endswith('.csv'):
...
#             # normalize data
#             transformed_df, scaler = normalize_data(df)

#             # create dataset with look_back window
#             X, y = create_dataset(transformed_df, look_back, horizon, mode)
...
#                 tmp_dict[key] = np.array_split(tmp_dict[key], num_batch)
#             data_dict[cid] = tmp_dict
#             data_dict[f'{cid}'] = scaler # for invert_transform
#             cid += 1

#     return data_dict

# # this one is used for NHITS from NeuralForecast only
# def _normalize_data(df:np.ndarray) -> np.ndarray:
#     scaler = StandardScaler()
#     return scaler.fit_transform(df)

# # create dataset with lookback window
# # this one is used for NHITS from NeuralForecast only
# def _create_dataset(df:np.ndarray, look_back:int)->tuple[np.ndarray, np.ndarray]:
#     size = df.shape[0] - look_back
#     X = np.array([df[i : i + look_back] for i in range(size)])
#     y = create_label(df)
#     y = y[look_back:]
...
# # this one is used for NHITS from NeuralForecast only
# def _get_data(look_back:int, test_size:float=0.8, batch_size:int=32, data_dir:str=RAW_DATA_DIR) -> dict[str, dict[str, list[np.ndarray]]]:
#     '''
...
#             # normalize data
#             transformed_df = _normalize_data(df)

#             # create dataset with lookback window
#             X, y = _create_dataset(transformed_df, look_back)
